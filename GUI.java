...
public class GUI {
    ...
    private void createMenubar() {
        ...
        JMenuItem resumeAndPauseGameItem = new JMenuItem("Pause/Resume game");
        gameMenu.add(resumeAndPauseGameItem);
        resumeAndPauseGameItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P, CTRL_SHIFT));
        resumeAndPauseGameItem.addActionListener(e -> pauseResumeButton.doClick());
        ...
        JMenuItem optionsGameItem = new JMenuItem("Options...");
        ...
        JMenu speedSecondMenu = new JMenu("Set speed");
        gameMenu.add(speedSecondMenu);

        JMenuItem slowItem = new JMenuItem("Slow");
        speedSecondMenu.add(slowItem);
        slowItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_1, CTRL));
        slowItem.addActionListener(e -> {
            slowButton.doClick();
            setSpeed(1);
        });

        JMenuItem medItem = new JMenuItem("Medium");
        speedSecondMenu.add(medItem);
        medItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_2, CTRL));
    ...
    private void applyOptions() {
        ...
        try{
            ...
            if(tollSize < 0 || riskRob < 0 || tollSize > 50 || riskRob > 50){
                ...
                if (tollSize < 0) {
                    sb.append("'Toll size' must be greater than or equals to 0.\n");
                }
                else if (tollSize > 50) {
                    sb.append("'Toll size' must be less than or equals to 100.\n");
                }
                if (riskRob < 0) {
                    sb.append("'Risk rob' must be greater than or equals to 0.\n");
                }
                else if (riskRob > 50) {
                    sb.append("'Risk rob' must be less than or equals to 100.\n");
                }
        ...
        try {
            ...
            if (min < 0 || max < 0 || min > 100 || max > 100) {
                StringBuilder sb = new StringBuilder();
                if (min < 0) {
                    sb.append("'Min. loss when robbed' must be greater than or equals to 0.\n");
                }
                else if (min > 100) {
                    sb.append("'Min. loss when robbed' must be less than or equals to 100.\n");
                }
                if (min < 0) {
                    sb.append("'Max. loss when robbed' must be greater than or equals to 0.\n");
                }
                else if (min > 100) {
                    sb.append("'Max. loss when robbed' must be less than or equals to 100.\n");
                }
                JOptionPane.showMessageDialog(mainFrame, sb.toString(), "Malformed input", JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(mainFrame, "'Min. loss when robbed' and 'Max. loss when robbed' must be integers.", "Malformed input", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    /**
     * Save your game as a log.
     * This method can save your game so you can see a replay of that later..
     */
    private void saveLog() {
        ...
        switch (showSaveDialog) {
            ...
            case JFileChooser.CANCEL_OPTION:
                return;
            default:
                JOptionPane.showMessageDialog(mainFrame, "An error happened when you tried to selected a file", "Error", JOptionPane.ERROR_MESSAGE);
                return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(mainFrame, e.getLocalizedMessage(), "An error happened when you tried to write in a file", JOptionPane.ERROR_MESSAGE);
        }
        try {
            if (FOS != null) {
                FOS.close();
            ...
            JOptionPane.showMessageDialog(mainFrame, e.getLocalizedMessage(), "An error happened when you tried t close the file", JOptionPane.ERROR_MESSAGE);
        }
    }
    
    /**
     * Play the logged game in the game.
     * This method can replay your saved game.
     */
    private void playLog() {
        int showPlayLog = fileChooser.showOpenDialog(mainFrame);
        switch (showPlayLog) {
            case JFileChooser.APPROVE_OPTION:
                break;
            case JFileChooser.CANCEL_OPTION:
        ...
        try {
            ...
            playedLog = (Log) OIS.readObject();
        } catch (IOException e) {
            JOptionPane.showMessageDialog(mainFrame, e.getLocalizedMessage(), "An error happened when you tried to open a file", JOptionPane.ERROR_MESSAGE);
        } catch (ClassNotFoundException e) {
            JOptionPane.showMessageDialog(mainFrame, e.getLocalizedMessage(), "An error happened when you tried to read a file", JOptionPane.ERROR_MESSAGE);
        } finally {
            try {
                if (FIS != null) {
                    FIS.close();
                ...
                JOptionPane.showMessageDialog(mainFrame, e.getLocalizedMessage(), "An error happened when you tried to close a file", JOptionPane.ERROR_MESSAGE);
            }
        }
        game.playLog(playedLog);
    }
